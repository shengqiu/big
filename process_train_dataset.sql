...
left join (
 ...
 from(
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date <= '2015-06-01'group by id
  ) h1
  left join (
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date > '2015-06-01' group by id
  )h2 on h1.id =h2.id
  left join (
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date <= '2015-09-01' and price_date > '2015-06-01' group by id
  )q3 on h2.id=q3.id
  left join (
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date > '2015-09-01' group by id
  )q4 on q3.id=q4.id
  left join (
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date = '2015-11-01' group by id
  )nov on q4.id=nov.id
  left join (
    select id, avg(price_p1_var) price_p1_energy, avg(price_p1_fix) price_p1_power, avg(price_p2_var) price_p2_energy, avg(price_p2_fix) price_p2_power, avg(price_p3_var) price_p3_energy, avg(price_p3_fix) price_p3_power from  bcg.train_hist where price_date = '2015-12-01' group by id
  )de on nov.id=de.id
)p
  on a.id = p.id
;